...
import com.nowui.fireelectronicsentry.utility.HttpDownloader;

public class BaseActivity extends AppCompatActivity implements OnRefreshListener<WebView> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initMap = (Map<String, Object>) getIntent().getSerializableExtra("initMap");
    ...
    private void setListenerEvent() {
        ...
        jockey.on(Helper.EventShowLoading + "-" + urlString, new JockeyHandler() {

            @Override
            public void doPerform(Map<Object, Object> payload) {
                System.out.println("jockey.on:" + Helper.EventShowLoading + " - " + urlString);
        ...
        jockey.on(Helper.EventHideLoading + "-" + urlString, new JockeyHandler() {

            @Override
            public void doPerform(Map<Object, Object> payload) {
                System.out.println("jockey.on:" + Helper.EventHideLoading + " - " + urlString);
        ...
        jockey.on(Helper.EventShowAlert + "-" + urlString, new JockeyHandler() {

            @Override
            public void doPerform(Map<Object, Object> payload) {
                System.out.println("jockey.on:" + Helper.EventShowAlert + " - " + urlString);

                new MaterialDialog.Builder(BaseActivity.this)
                        .title(getResources().getString(R.string.dialog_title_tips))
                        .content(payload.get("content").toString())
                        .positiveText(getResources().getString(R.string.dialog_agree))
    ...
    private class BaseWebViewClient extends WebViewClient {
            ...
            if(isLocal) {
                ...
                for (Map<String, String> item : list) {
                    if (url.contains(item.get(Helper.KeyName))) {
                        ...
                        System.out.println("----------------------");
                        System.out.println(tempUrl.replace(Helper.WebUrl + "/", "").replace(Helper.WebUrl, "").replace("http://oa.herigbit.com.cn/", "").replace("http://leon.tunnel.mobi/", ""));

                        try {
                            response = new WebResourceResponse(item.get(Helper.KeyMimeType), "utf-8", getAssets().open(tempUrl.replace(Helper.WebUrl + "/", "").replace(Helper.WebUrl, "").replace("http://oa.herigbit.com.cn/", "").replace("http://leon.tunnel.mobi/", "")));
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
        ...
        public void onPageFinished(WebView view, String url) {
            ...
            jockey.send(Helper.EventInitMap + "-" + urlString, webView, initMap);
        }
